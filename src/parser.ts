...
export class Parser {
    ...
    private static getDetailedExplanation(rule: string, message: string, source?: string): string {
        ...
        if (ruleLower === 'truthy' || ruleLower.includes('truthy')) {
            explanation += '\n💡 Используйте true/false вместо yes/no/on/off';
            ...
            explanation += '\n💡 Проверьте количество пустых строк между блоками';
            ...
            explanation += '\n💡 Исправьте формат комментариев (пробел после #)';
        } else if (ruleLower === 'colons' || ruleLower.includes('colons')) {
            explanation += '\n💡 Проверьте пробелы вокруг двоеточий (key: value)';
        } else if (ruleLower === 'braces' || ruleLower.includes('braces')) {
            explanation += '\n💡 Проверьте пробелы внутри фигурных скобок { }';
        } else if (ruleLower === 'brackets' || ruleLower.includes('brackets')) {
            explanation += '\n💡 Проверьте пробелы внутри квадратных скобок [ ]';
        } else if (ruleLower === 'hyphens' || ruleLower.includes('hyphens')) {
            explanation += '\n💡 Проверьте пробелы после дефисов в списках';
            ...
            explanation += '\n💡 Удалите дублирующийся ключ';
        }

        // ANSIBLE-LINT правила
        else if (ruleLower.includes('fqcn')) {
            explanation += '\n💡 Используйте полное имя модуля (FQCN), например: ansible.builtin.apt вместо apt';
            ...
            explanation += '\n💡 Добавьте "changed_when:" для команд shell/command';
        } else if (ruleLower.includes('risky-file-permissions')) {
            explanation += '\n💡 Укажите явные права доступа (mode: "0644")';
        } else if (ruleLower.includes('command-instead-of-shell')) {
            explanation += '\n💡 Используйте модуль command вместо shell если не нужны фичи shell';
        } else if (ruleLower.includes('command-instead-of-module')) {
            explanation += '\n💡 Используйте специализированный модуль вместо команды';
        } else if (ruleLower.includes('package-latest')) {
            explanation += '\n💡 Используйте конкретную версию пакета вместо state: latest';
        } else if (ruleLower.includes('deprecated')) {
            explanation += '\n💡 Модуль или синтаксис устарел - используйте новую версию';
        } else if (ruleLower.includes('role-name')) {
            explanation += '\n💡 Имя роли должно соответствовать стандартам (lowercase, буквы и цифры)';
        } else if (ruleLower.includes('meta-no-info')) {
            explanation += '\n💡 Добавьте meta/main.yml с информацией о роли';
        ...
        // PRE-COMMIT правила
            ...
            explanation += '\n💡 Удалите маркеры конфликта слияния (<<<<, ====, >>>>)';
        } else if (ruleLower === 'mixed-line-ending') {
            explanation += '\n💡 Используйте единообразные окончания строк (LF или CRLF)';
        } else if (ruleLower === 'check-case-conflict') {
            explanation += '\n💡 Имена файлов различаются только регистром - возможны проблемы на Windows/Mac';
        } else if (ruleLower === 'debug-statements') {
            explanation += '\n💡 Удалите отладочные операторы (print, debugger, etc.)';
        }
